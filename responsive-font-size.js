function responsiveFontSize(selector, minFont, maxFont){
		...
		document.querySelectorAll(selector).forEach(function(elem){
			...
			var smaller_width = elem.getBoundingClientRect().width;
			elem.style.fontSize = "2px"
			var bigger_width = elem.getBoundingClientRect().width;
			...
			//Set initial size
			var container_width = parseFloat(window.getComputedStyle(elem.parentElement, null).getPropertyValue('width'));
			var fontSize = (container_width -1) / pixel_change;
			if(fontSize <= minFont){
		...
		function(event){
			elems.forEach(function(elem){
				//Set correct size
				var container_width = parseFloat(window.getComputedStyle(elem["parent"], null).getPropertyValue('width'));
				var fontSize = (container_width -1) / elem["pixel-change"];
				if(fontSize <= minFont){
